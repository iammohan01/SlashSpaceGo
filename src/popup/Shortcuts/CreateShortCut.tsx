...
export default function CreateShortCut(): React.ReactElement {
    ...
    useEffect(() => {
        ...
        generateCurrentTabData(key, target)
            .then((currentTabData) => {
            ...
            .catch((err) => {
                ...
                message
                    .error(
                        'Something went wrong while fetching current tab data',
                        3
                    )
                    .then();
            });
    ...
    function editAndUpdateShortcut() {
        ...
        updateShortcut(updatedShortcut, true)
            .then((updatedValues) => {
    ...
    function initSaveShortcut() {
        ...
        if (trimmedKey) {
            ...
            saveShortcut({...currentTabData, target: target})
                .then((s) => {
                    if (setShortcutsInContext != null) {
    ...
    return (
        <div
            ...
            className="create-shortcut-wrapper"
        >
            {contextHolder}
            {!!showToast && showToast}
            ...
            <div className={'w-[80%] border-b mb-[0.6rem]'}>
                <Input
                    ...
                    variant="borderless"
                    onChange={changeUrl}
                    value={editMode ? urlKey : url}
                />
            </div>
            <p className={'flex justify-between gap-1'}>
                <input
                    id={'open-in-previous-tab'}
                    checked={target == UrlTarget.IN_EXISTING_TAB}
                    onChange={(event) => {
                    ...
                    type={'checkbox'}
                />
                <label htmlFor={'open-in-previous-tab'}>
                    Open in already opened tab
                </label>
            </p>
