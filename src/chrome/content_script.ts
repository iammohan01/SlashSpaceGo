                    ...
                    if (data) {
                        if (inputForRequest != null && target != null) {
                            if (isInputFiled || isTextArea) {
                                ...
                                const indexOfKeyInInput = value.lastIndexOf(
                                    data[0].key
                                );
                                if (indexOfKeyInInput !== -1) {
                                    const updatedInput =
                                        value.substring(0, indexOfKeyInInput) +
                                        data[0].value +
                                        value.substring(
                                            indexOfKeyInInput +
                                            data[0].key.length
...
function addInputListenerForMutated(node: {
    ...
    addEventListener: (
        arg0: string,
        arg1: { (e: Event): void; (e: Event): void }
    ) => void;
    hasAttribute: (arg0: string) => any;
}) {
    if (
        node.tagName === 'INPUT' ||
        (node.tagName === 'DIV' && node.hasAttribute('contenteditable'))
    ) {
